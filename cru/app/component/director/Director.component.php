...
<script>
    $.getJSON('https://whitehurricane.000webhostapp.com/service2.php', function(data) {
        for (var i = 0; i < data.length; i++) {
            ...
            $("#lim_path" + i + "").html("<a class='btn btn-sm bg-teal'>0 / 5 </a>" +
                <?php if ($cs == 1) { ?> "<a href='?page=2&director_select=" + data[i].Name + "'" +
                    "class='btn btn-sm btn-primary ml-3'>" +
                    "<i class = 'fas fa-user'> เลือกกรรมการ </i>" +
                    "</a>"
                <?php } else { ?> "<a href='session/?research=1&name=" + data[i].Name + "'" +
                    "class='btn btn-sm btn-primary ml-3'>" +
